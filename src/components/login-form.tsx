        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormDescription>This is the email you used to sign up</FormDescription>
              <FormMessage />
            </FormItem>
          )}
        />
            ...
            <FormItem>
              ...
              <FormControl>
                <InputOTP
                  ...
                  onChange={(value) => form.register("twoFactorCode").onChange({ target: { value }, type: "change" })}
                >
                  {Array.from({ length: Math.ceil(twoFactorLength / (twoFactorLength % 3 === 0 ? 3 : 2)) }).map(
                    (_, groupIndex) => (
                      <InputOTPGroup key={groupIndex}>
                        {Array.from({
                          length: Math.min(
                            twoFactorLength - groupIndex * (twoFactorLength % 3 === 0 ? 3 : 2),
                            twoFactorLength % 3 === 0 ? 3 : 2
                          ),
                        }).map((_, slotIndex) => (
                          <InputOTPSlot
                            key={slotIndex}
                            index={groupIndex * (twoFactorLength % 3 === 0 ? 3 : 2) + slotIndex}
                          />
                        ))}
                      </InputOTPGroup>
                    )
